...
try {
    ...
    $sql = "SELECT item_id,content FROM $table ORDER BY item_id DESC";
    $result = $db->query($sql);
    ...
    foreach ($result as $row) {
        ...
        $content = $row['content'];
        echo "
        <div class=\"flex w-full bg-neutral-800 rounded-lg hover:scale-[1.01] shadow-sm p-5 space-x-1 items-center transition-all ease-in-out\" id=\"item-" . $item_id . "\" key=\"item-" . $item_id . "\">
